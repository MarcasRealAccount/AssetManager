...
#include "ResourceManager/Impl/WeakRef.h"
...
namespace ResourceManager
{
	...
	void ResourcePool<T, IndexType>::incRef(IndexT index)
	{
	...
	template <class T, class IndexType>
	void ResourcePool<T, IndexType>::decRef(IndexT index)
	{
		auto ptr = getResourcePtr(index);
		if (ptr && ptr->decRef())
			erase(index);
	...
	template <class T, class IndexType>
	void ResourcePool<T, IndexType>::incWRef(IndexT index)
	{
		auto ptr = getResourcePtr(index);
		if (ptr)
			ptr->incWRef();
	...
	template <class T, class IndexType>
	void ResourcePool<T, IndexType>::decWRef(IndexT index)
	{
		auto ptr = getResourcePtr(index);
		if (ptr && ptr->decWRef())
			erase(index);
	...
	template <class T, class IndexType>
	template <class... Args>
	ResourcePool<T, IndexType>::Ref ResourcePool<T, IndexType>::emplaceBackPersistent(Args&&... args) requires std::is_constructible_v<T, Args...>
	{
		Ref  ref      = emplaceBack(std::forward<Args>(args)...);
		auto resource = getResourcePtr(ref.index());
		resource->setPersistent();
		return ref;
	}
